...
moterist.com（および vodnavi.jp 再構築時）で使用する **THE THOR テーマの装飾構文を即引用できる辞書**。
本辞書に存在しない装飾は記事に混入させないこと。世界観（ダーク × ゴールド：`#121212` / `#E0E0E0` / `#D4AF37`）の崩壊と、エディタの自動整形による HTML 破壊を同時に防ぐ。
生成した記事は **クラシックエディタ形式の生 HTML** として出力すること（Gutenberg ブロックコメント `<!-- wp:... -->` は禁止）。
...
3. **インラインスタイル直書きは原則禁止**：色は本ガイドで指定されたクラス・既存ショートコードを経由する。理由：ブランド色変更時に全記事を一括差し替えできる状態を保つ。
...
<div class="btn btn-center">
  <a class="btn__link btn__link-primary"
     href="https://al.dmm.co.jp/?lurl=…&af_id={AFFILIATE_ID}&ch=link_tool&ch_id=link"
     target="_blank"
...
```
- 文言：「確認する」志向で統一。
- `rel="noopener sponsored"` は必須（コンプラ）。`nofollow` は付けない。
- `af_id`・`ch`・`ch_id` は必ず保持（成果トラッキング）。`{AFFILIATE_ID}` はテンプレ変数で、本番時は環境変数・共通定数から差し込まれる。**直書き禁止**。
...
```
- `?source=moterist` + `&intent=<beginner|actress|discount>` を必ず付ける。
- intent 未指定は差し戻し対象。
...
<div class="btn btn-center btn-double">
  <a class="btn__link btn__link-primary"
     href="https://al.dmm.co.jp/…/{CONTENT_ID}/…?af_id={AFFILIATE_ID}&ch=link_tool&ch_id=link"
     target="_blank" rel="noopener sponsored">
    作品ページで詳細を確認する
  </a>
  <a class="btn__link btn__link-tertiary"
     href="https://al.dmm.co.jp/…/-/list/=/?searchstr={ACTRESS_OR_SKU}&af_id={AFFILIATE_ID}&ch=link_tool&ch_id=link"
     target="_blank" rel="noopener sponsored">
    出演女優の他作品を一覧で見る
  </a>
</div>
```
- 用途：作品ページが配信終了で 404 になった場合のセーフティネット。
- `{CONTENT_ID}` / `{ACTRESS_OR_SKU}` / `{AFFILIATE_ID}` はテンプレ変数。記事生成時に展開する。
...
```
- アンカリング配置：**最高単価を左端**、本命（FANZA）を真ん中または右に置く。
- 5 行以上は分割（情報過多で離脱）。
...
- 記事生成時に提示する。実際の埋め込みは THE THOR の記事編集画面または All in One SEO 経由（値だけ提案する）。
- 推奨長：90〜120 字。冒頭にキーワードを置き、行動を促す動詞で終える。

### 10.2 サイト内インデックス制御
- `noindex` / `canonical` は **記事側で設定しない**。SEO 不変ルールに従い、変更が必要な場合のみ個別指示で対応。
...
| インラインスタイル `style="color: #ff0000"` | ブランド色変更時の一括差替を阻害（禁則色との衝突） |
| 純白 `#FFFFFF` / 純黒 `#000000` / ネオン系の直書き | 世界観破壊 |
...
| 架空の口コミ / 偽セール表記 | 景表法・規約違反 |
| `af_id=` の値を直書き | アフィリエイト ID 厳格分離ルール違反 |

---

## 12. 出力チェックリスト

記事原稿を出力する直前に以下をセルフチェックする：
...
- [ ] アフィリエイト ID の値を直書きしていない（`{AFFILIATE_ID}` 等のテンプレ変数で記述）
