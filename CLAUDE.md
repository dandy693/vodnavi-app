...
**指標①（2026-08-11 に「クリック実数30件」から**シグナル3点**へ再改訂）… ①-a works→articles 内部リンククリック **≥1件**（導線が物理的に機能するか＝**主判定**）/ ①-b articles 表示回数 **≥50/月** / ①-c articles面アフィリエイトクリック **≥1件** ／ **②`/articles/`宛のDofollow DR30以上 +2件（変更なし）** / ③9月単月報酬 15,000円** で判定する。**①-a が 0 のまま9月末を迎えたら articles 経路を12月目標から外す**（事前登録）。**本ゲートはシグナル判定であって収益テストではない**。**9月末に数値を見てからの指標変更・閾値調整は禁止**。中間測定は 8/31（記録のみ）。判定ルール・基準線・交絡要因の全文 → `management/_metrics/GATE_20260930.md`
