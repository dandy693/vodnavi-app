...
- **将来パス→申請確定(2026-07-29 CSO裁定で更新)**: soft-keep専用af_idは**8月頭に第2ファネル用(§12)と一括申請**(発行操作=HUMAN枠・CSO実施・申請情報=§12-2)。**発行までは(b)生URLを維持**し、発行後のfallbackUrl切替は層B確定後のCSO承認を経たコード変更として実施(990系=API専用と同じ「用途別ID分離」の設計思想)。
...
2. **af_id=案B(第2ファネル専用af_id新規取得)【CSO裁定確定 2026-07-29】**: 理由(CSO)——DMMレポートはaf_id単位でしか分解できず、004共用ではTV Plus CTAクリックが**004のEPC分母(180cl・16.4円/cl)を汚染**する。成果側が商品別に分解できてもクリック側が分離不能では評価に使えない。用途別ID分離(990系・soft-keep用)の既存思想と整合。**申請は8月頭に一括(第2ファネル用+soft-keep用=§10)・発行操作=affiliate.dmm.com上の書き込み=HUMAN枠(CSO実施)**。CTOは申請情報パッケージを事前提示(下記§12-2)・発行後にaf_id台帳へ追記。
3. **placement命名**: 新規値=`guide_tvplus_add_cta`(案)。既存カスタムディメンション枠への**値追加のみ=GA4側設定変更不要**・既存値guide_tv_signup_ctaは不変。
...
### §12-2. af_id新規申請 情報パッケージ(8月頭一括申請用・発行操作=HUMAN枠=CSO実施)
...
### 実装
- 8月頭CSO承認後・**PR-5(独立)**。①〜④と同時実行可だが、着地URL(実査①)と**af_id発行(007)**が前提入力。
