...
2. **af_id(裁定保留・8月頭までにCSO決定)**: 案A=004共用——DMMレポートの報酬別/サービス別でTV(¥2,750)とTV Plus(¥2,200)がカテゴリ分解できるなら系列は保てる(可否はレポート画面で要確認)。案B=**第2ファネル専用af_idの新規取得**——「af_id単位でしか分解できない」制約(fallbackUrl裁定の理由)への恒久解であり、soft-keep専用af_id(§10将来パス)と合わせて8月頭に一括申請を検討するのが整合的。取得リードタイムが公開時期に影響する場合は案Aで先行し後日切替も可(切替時は新規placement同様に系列非接触)。
...
### 実装
- 8月頭CSO承認後・**PR-5(独立)**。①〜④と同時実行可だが、着地URL(実査①)とaf_id裁定が前提入力。
