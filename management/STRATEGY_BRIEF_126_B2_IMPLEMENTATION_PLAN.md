...
## 10. C2方針案(API消失作品のworksページ挙動・同梱)
- 現状: APIデリスト作品(例ebwh00359)は404(SEV-1対応後の挙動)。3案比較:
  - (a) 410+sitemap除外: クロール資源は節約されるがリンクエクイティ・再配信復活時の回復を放棄
  - **(b) soft-keep(推奨)**: キャッシュ済みメタで「配信終了」表示+検索フォールバックCTA(url-builder既存のfallbackUrl=女優名検索アフィリンク)を提示。収益導線とインデックスを維持し、再配信時に自動復帰。sitemap-archiveは現状維持(DB突合1,418一致の仕組みを流用)
  - (c) 404現状維持: 実装ゼロだがGSC 404蓄積(過去237件問題の再演リスク)
- (b)の実装はキャッシュデータの保持期間・「配信終了」表記の台帳整合(価格・特典表示は落とす)を条件に、PR-2へ同梱可能な規模。**採否はCSO裁定待ち**。
