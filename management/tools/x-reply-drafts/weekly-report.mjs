...
//
// CLI:
//   node weekly-report.mjs --replies replies.json --targets targets.json [--since 2026-09-18] [--until 2026-09-24] [--md]
...
function bucket() {
  return { count: 0, draft_used: { A: 0, B: 0, C: 0, "空": 0 }, got_like: 0, got_reply: 0, profile_click_delta_filled: 0, no_post_id: 0 };
}
 ...
 */
export function aggregate({ replies, targets, since = null, until = null }) {
  ...
  const out = { period: { since, until }, total: bucket(), by_priority: {}, by_type: {}, by_priority_type: {}, by_target: {}, unmatched: [] };
  for (const r of rows) {
    ...
    add(out.by_priority[pri], r);
    (out.by_type[type] ??= bucket());
    add(out.by_type[type], r);
    const k = `${pri}｜${type}`;
    (out.by_priority_type[k] ??= bucket());
    add(out.by_priority_type[k], r);
    (out.by_target[handle] ??= bucket());
    add(out.by_target[handle], r);
  }
  return out;
}

function fmtBucket(b) {
  return `${b.count} | A ${b.draft_used.A} / B ${b.draft_used.B} / C ${b.draft_used.C}${b.draft_used["空"] ? ` / 空 ${b.draft_used["空"]}` : ""} | ${b.got_like} | ${b.got_reply} | ${b.profile_click_delta_filled}${b.no_post_id ? ` | reply_post_id 空 ${b.no_post_id}` : ""}`;
}
...
export function toMarkdown(agg) {
  ...
  L.push("");
  L.push("| 区分 | 件数 | draft_used | got_like ✓ | got_reply ✓ | profile_click_delta 記入 |");
  L.push("|---|---|---|---|---|---|");
  L.push(`| **合計** | ${fmtBucket(agg.total)} |`);
  for (const [k, b] of Object.entries(agg.by_priority).sort()) L.push(`| priority ${k} | ${fmtBucket(b)} |`);
  for (const [k, b] of Object.entries(agg.by_type).sort()) L.push(`| type ${k} | ${fmtBucket(b)} |`);
  L.push("");
  L.push("| priority｜type | 件数 | draft_used | got_like ✓ | got_reply ✓ | profile_click_delta 記入 |");
  L.push("|---|---|---|---|---|---|");
  for (const [k, b] of Object.entries(agg.by_priority_type).sort()) L.push(`| ${k} | ${fmtBucket(b)} |`);
  L.push("");
  L.push("| 対象 | 件数 | draft_used | got_like ✓ | got_reply ✓ | profile_click_delta 記入 |");
  L.push("|---|---|---|---|---|---|");
  for (const [k, b] of Object.entries(agg.by_target).sort((a, b2) => b2[1].count - a[1].count || (a[0] < b2[0] ? -1 : 1))) L.push(`| ${k} | ${fmtBucket(b)} |`);
...
if (isMain(import.meta.url)) {
  ...
  if (!a.replies || !a.targets) {
    process.stderr.write("usage: node weekly-report.mjs --replies replies.json --targets targets.json [--since YYYY-MM-DD] [--until YYYY-MM-DD] [--md]\n");
    process.exit(2);
  }
  const agg = aggregate({
