...
| 1（旧・**補完用に残す**） | **CTO**（CSO 指示 2026-09-19 08:2x で HUMAN → CTO へ改訂・FACT §26-10-1） | Chrome 抽出（**1 日 2 回＝朝 06:00・夜 21:00 前**（**CSO 決定 2026-09-21 で朝 08:00 → 06:00／CSO判定 2026-09-24 で夜 21 時前 → 22:30 → 【再訂正・CSO裁定 2026-09-24】夜は 21:00 前に戻す（HUMAN の投稿時間に合わせる）。22:30 の抽出は「HUMAN が指示した日のみの任意追加」とし既定にしない**。旧時刻は訂正として残す）・窓は**前回抽出以降**＝夜 21:00 前 → 翌朝 06:00・読み取り専用＝投稿・返信・フォロー・いいね・ブックマークをしない）→ `@ハンドル｜投稿日時｜投稿URL｜本文｜リンク先 content_id` を `runs/<日付>/input.txt` に置く（本文は台帳に貼らない）。重複はツール側が `target_post_url` で排除 |
...
| **2.5**（**記録の後**・CSO 指示 2026-09-24） | **CTO** | **フォロー営業の候補提示**: 当日リプした対象投稿の「いいね／返信」から**一般ユーザー 5〜10 件**を読み取り専用で拾い、`follow-candidates.mjs --candidates` で絞り込んで**ハンドル＋根拠**を提示（メーカー・女優・他アフィリエイター・自動投稿は除外し、除外件数のみ報告）。**【取得元の変更・CSO裁定 2026-09-24 夜】取得元は (a) 稼働の女優本人アカウントの直近投稿（返信数の多いもの・私生活投稿でも可）の返信欄、(b) 対象投稿の「リポスト」一覧（`/status/<id>/retweets`・引用 `/quotes` を含む）。いいね一覧は使わない（他人の投稿では見られない）。上の「いいね／返信」は訂正として残す。初回提示は 2026-09-25 朝の抽出後。****HUMAN が 1 日 10 件までフォロー**（CTO はフォローしない）→ フォローした分を `--record` で `follows.json` に追記。**推定フォロー中が 300 に達したら停止して報告**（正は HUMAN の実測・FACT §26-13） |
| 3 | HUMAN | 案を選んで投稿 → リプ URL を Claude Code に貼る。**夜の投稿は 21:00 前の抽出後**（CSO判定 2026-09-24 の 4 の「22:30〜23:00」は同日の CSO裁定で再訂正・22:30 抽出は HUMAN 指示日のみの任意追加） |
...
0. **対象リスト**（CTO）: `x_targets` を MCP で読み戻して `state/<日付>/targets.json` に保存 → `node management/tools/x-reply-drafts/active-targets.mjs state/<日付>/targets.json --urls` の一覧を Chrome 抽出の巡回先にする（`--json` で内訳）。手順 3 の `targets.json` にもこのファイルを使える。
   ...
   - **【一斉発売日の提示上限・CSO裁定 2026-09-25 朝】同日に「本日先行配信スタート」型の投稿が 5 件以上あるときは、提示を最大 4 件に絞る。同じ質問（例: 通常配信までの期間）は 1 日 1 回まで。残りは提示せず件数のみ報告する。** ツールの停止判定には入れず CTO が提示時に絞る（入力の並び＝priority 順で先頭 4 件・C 案が同じ質問に重なる場合は 2 件目以降の C を外す）。実例＝2026-09-25 朝は同型 9 件で 10 行・C 案 9 件がほぼ同一の質問になった。
