 ...
 *
 * 【rev2 の残課題と rev3（案C'）】
 *   rev2 は `400-999` を「安い順」パスへ割り当てていたが、**232/1,500 で未充足**に終わった。
 *   停止したのは `MAX_CALLS` ではなく **`offset <= 50_000` の上限**である。
 *   実測: **`total_count` は 50,000＝API 上限に張り付いており**、
 *   **`sort=-price` offset=49901 の価格は 300円（まだ 0-399 帯）**＝
 *   **安い順では5万件走査しても 400-999 帯にほとんど到達しない。**
 *   一方 **`sort=price` offset=15001 は 686〜980円で、100件中100件が 400-999 該当**
 *   （offset=10001 で 740〜1,480円・2件 / offset=9401 で 1,848〜1,980円・0件）。
 *
 *   rev3（案C'）: **`400-999` を高い順パスへ移す**。
 *     - `sort=price`（高い順）  … 3000+ / 2000-2999 / 1000-1999 / **400-999**
 *     - `sort=-price`（安い順） … 0-399
 *   高い順は 94コールで offset 約9,400（1,000円台）まで到達済みのため、
 *   そこから 400-999 帯へは連続して到達できる。
 *
 ...
 *   - API コール: **BRIEF_128 の前提「約50〜70回」は満たせない**（第82便 CSO裁定(2) で訂正）。
 *                 **50,000件の窓を価格順に走査する構造上、帯の位置までページングを要する。**
 *                 実測 → rev1 28コール（設計の38%）/ rev2 594コール（74.6%）/ rev3 は本文に記録。
 *
...
const PASSES = [
  {
    ...
    label: "高い順",
    bands: ["3000+", "2000-2999", "1000-1999", "400-999"],
    beyond: (price) => price < 400,
  },
  {
    sort: "-price", // 価格の安い順（2026-08-21 実測）
    label: "安い順",
    bands: ["0-399"],
    beyond: (price) => price > 399,
  },
];
