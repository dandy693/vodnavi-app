...
| **第61便 補遺4** | 08-17 | 同上 | **症状なし。補遺3 で失敗した3ページすべてを取得できた**（`cloud-resource-manager` / `projectcreate` / `billing` / GA4 `admin` / GA4 `suiteusermanagement/property`。**`get_page_text` と `screenshot` の両方が成功**） | **回復。ただし何をしたから回復したのかは特定していない。** **観測できた差分は「タブ数が 8 → 2 に減っていた」ことのみ**（CTO はタブを閉じていない）。**Chrome プロセスは 42・合計 1.04GB・最古プロセスの連続稼働 96時間で、再起動は挟まれていない。** **【厳守】「タブを減らしたから直った」と読まない**——第31→32便で「再起動したから直った」と読まなかったのと同じ姿勢（本ログ「事実として言えること」4）。**時間の経過という差分も同時に存在する** |
