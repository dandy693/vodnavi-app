...
// (a) CSV
const cols = ["handle", "display_name", "type", "priority", "status", "source", "genres", "note"];
...
console.log(JSON.stringify({ count: records.length, excluded: seed.meta.excluded.length, byType: by("type"), byPriority: by("priority"), genresUsed: [...new Set(records.flatMap((x) => x.fields.genres ?? []))].sort(), batches: batches.length }, null, 2));
