...
- **検査対象**: 116 件（配信済み 98 / 未配信 18）
- **検査したガード**: 18 件 ＋ 追加検査 2 件
- **違反**: **9 件**（配信済み 9 / 未配信 0）
...
- **p3_link_reachable** … **未実行**。`--check-links` を付けるとリンク先の実在を検査する（既定は行のデータのみ）
- **予約日時が未設定のため判定不能**: 4 件
  - B8 T1改 園田茉莉華 デビュー作 / g6_one_affiliate_per_day
  - B8 T1改 園田茉莉華 デビュー作 / g8_time_window
  - B8 T1改 園田茉莉華 デビュー作 / g9_utc_iso
  - B8 T1改 園田茉莉華 デビュー作 / g11_one_work_intro_per_day
...
| `recybssEC5Alcnzl4` | W7-05 小ネタ 見放題の確かめ方 | 投稿済 | 2026-08-17 22:30 | `p1_postid_missing` | 投稿済だが ポストID が無い（X 上の実在は目視確認を要する） |
