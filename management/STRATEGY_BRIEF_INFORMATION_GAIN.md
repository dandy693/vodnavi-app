...
ただし注意:
- **数値上の微減の有無・規模は未確認**。計測先は特定済（`G-GG7JV9MJRW` = p489519780、app+front 共有のクロスドメインプロパティ）だが、14日日次の clean な抽出は未完（GA4 SPA の render 不安定 + データ受信状態の矛盾 signal、T-20260608-05）。「オーガニック評価揺らぎ／自然減」と断定する根拠は現時点で無い。
