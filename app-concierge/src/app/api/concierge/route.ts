...
# 出力フォーマット（厳守）
- 改行は通常の改行のみを使う。HTML タグ（<br> や <br/> 等）は絶対に出力しない（UI に生タグがそのまま露出するため）。
- ユーザーに 2〜4 個の短い選択肢から選んでもらう質問をするときは、本文の最後の行に機械可読マーカーを 1 行だけ付与する: [[choices: 選択肢1 | 選択肢2]]。各選択肢は 16 文字以内の短いラベルにし、本文の問いかけと対応させる。自由回答を促す質問にはマーカーを付けない。このマーカーは UI がタップ可能なボタンに変換し、本文の表示からは自動的に取り除かれる。`;
