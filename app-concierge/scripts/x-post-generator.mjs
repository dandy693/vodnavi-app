...
export const ACTRESS_LAST_POSTED = {
  ...
  // ── 2026-08-21 追記（Airtable 実測）。本表は 2026-08-13 実測のまま更新されておらず、
  //    **その後に実配信された3件が欠落していた**。g12 はこの表だけを見るため、
  //    **表が古いと直近に登場した女優が素通りする**。
  //    登録するのは **配信済み（ポストIDあり）または予約済み（承認済＋予約日時あり）** のみとし、
