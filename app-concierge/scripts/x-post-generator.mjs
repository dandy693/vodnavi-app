...
export const ACTRESS_ENTRY_SOURCE = {
  "松永あかり": "recUxVPuEsSGOdZ7S",
  "小笠原菜乃": "reca8WXGmkS1wztwV",
};
...
/**
 * 既存 TG の slug 別 最終使用日（**実測 2026-09-10・Airtable の配信実績で更新**）。
 *
 * 【手で更新している。**`ACTRESS_LAST_POSTED` と同じ構造の危うさを持つ**】
 * `g16` はこの表だけを見るため、**古いままだと直近に使った記事が検査を素通りする。**
 * 2026-08-28 の照合で **2件が実績より古いまま**だった。
 * **2026-09-10 の照合では W9 バッチの TG-16〜TG-21（6件）が丸ごと欠落していた**
 * ——8/28 以降 TG を7件投入したが、本表を一度も直していなかった。
 ...
 *
 * 【登録基準】`ACTRESS_LAST_POSTED` と揃える——**配信済み（投稿済＋ポストID）**
 * または **予約済み（承認済＋予約日時）** のみ。**ストックは登録しない。**
 * したがって `fanza-first-guide` は TG-15（W9-15・2026-08-29 予定）が
 * **配信されずストックへ戻された**ため 2026-08-04 のまま（§13-6-6）。
 */
export const TG_LAST_USED = {
  // TG-15（W9-15・2026-08-29 予定）は 8/29 の配信障害で未配信＝ストックのため登録しない
  "fanza-first-guide": "2026-08-04",
  // TG-20（2026-09-08 配信・ポストID 2097316565930660102）
  "fanza-tv-free-trial": "2026-09-08",
  // TG-21（2026-09-10 22:30 予約済・未配信）。予約済みは登録する
  "fanza-kaiyaku": "2026-09-10",
  // TG-16（2026-08-31 配信・ポストID 2094417500339363919）
  "fanza-payment-methods": "2026-08-31",
  // TG-17（2026-09-02 配信・2095142233317986621）／ TG-18（2026-09-04 配信・2095867058118398395）
  "fanza-payment-statement": "2026-09-02", "fanza-tv-guide": "2026-09-04",
  // TG-19（2026-09-06 配信・ポストID 2096591782532247824）
  "fanza-tv-review": "2026-09-06",
  // fanza-subscription-vs-single-purchase は TG 未使用（記事A・観測窓〜9/12 のため本バッチでも使わない）
};
